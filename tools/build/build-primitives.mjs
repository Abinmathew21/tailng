...
const distRoot = resolve(workspaceRoot, 'dist/libs/tailng-ui/primitives');
...
if (!ngcPath) {
  console.error('build-primitives: could not locate Angular compiler-cli ngc binary.');
  process.exit(1);
}

if (!existsSync(cdkTypesEntry)) {
  console.error(
    'build-primitives: missing dist/libs/tailng-ui/cdk/src/index.d.ts. Build cdk first.',
  );
  process.exit(1);
}
...
const tsConfigPath = resolve(workspaceRoot, 'libs/tailng-ui/primitives/tsconfig.lib.ngc.json');
...
cpSync(
  resolve(workspaceRoot, 'libs/tailng-ui/primitives/package.json'),
  resolve(distRoot, 'package.json'),
);
cpSync(
  resolve(workspaceRoot, 'libs/tailng-ui/primitives/README.md'),
