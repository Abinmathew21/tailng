...
type DrawerDocSectionId = 'api' | 'examples' | 'overview' | 'ownable-install' | 'styling';

const drawerDocSectionIds: readonly DrawerDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
const defaultDrawerDocSection: DrawerDocSectionId = 'overview';

function isDrawerDocSectionId(value: string): value is DrawerDocSectionId {
  return drawerDocSectionIds.includes(value as DrawerDocSectionId);
}
...
export class DrawerPageComponent {
  ...
  public readonly activeSection = computed<DrawerDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultDrawerDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isDrawerDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): DrawerDocSectionId | null {
    ...
    if (section === undefined || !isDrawerDocSectionId(section)) {
      return null;
    }
