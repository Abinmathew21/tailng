...
function isCardDocSectionId(value: string): value is CardDocSectionId {
  return cardDocSectionIds.includes(value as CardDocSectionId);
}
...
export class CardPageComponent {
  ...
  public readonly activeSection = computed<CardDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultCardDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isCardDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): CardDocSectionId | null {
    ...
    if (section === undefined || !isCardDocSectionId(section)) {
      return null;
    }
