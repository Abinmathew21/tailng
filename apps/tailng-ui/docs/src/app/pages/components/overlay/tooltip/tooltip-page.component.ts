...
type TooltipDocSectionId = 'api' | 'examples' | 'overview' | 'ownable-install' | 'styling';

const tooltipDocSectionIds: readonly TooltipDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
const defaultTooltipDocSection: TooltipDocSectionId = 'overview';

function isTooltipDocSectionId(value: string): value is TooltipDocSectionId {
  return tooltipDocSectionIds.includes(value as TooltipDocSectionId);
}
...
export class TooltipPageComponent {
  ...
  public readonly activeSection = computed<TooltipDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultTooltipDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isTooltipDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): TooltipDocSectionId | null {
    ...
    if (section === undefined || !isTooltipDocSectionId(section)) {
      return null;
    }
