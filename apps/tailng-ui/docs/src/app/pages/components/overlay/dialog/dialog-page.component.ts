...
type DialogDocSectionId = 'api' | 'examples' | 'overview' | 'ownable-install' | 'styling';

const dialogDocSectionIds: readonly DialogDocSectionId[] = [
  'overview',
  'api',
  'styling',
  'examples',
...
const defaultDialogDocSection: DialogDocSectionId = 'overview';

function isDialogDocSectionId(value: string): value is DialogDocSectionId {
  return dialogDocSectionIds.includes(value as DialogDocSectionId);
}
...
export class DialogPageComponent {
  ...
  public readonly activeSection = computed<DialogDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultDialogDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isDialogDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): DialogDocSectionId | null {
    ...
    if (section === undefined || !isDialogDocSectionId(section)) {
      return null;
    }
