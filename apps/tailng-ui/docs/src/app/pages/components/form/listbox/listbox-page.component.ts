...
function isListboxDocSectionId(value: string): value is ListboxDocSectionId {
  return listboxDocSectionIds.includes(value as ListboxDocSectionId);
}
...
export class ListboxPageComponent {
  ...
  public readonly activeSection = computed<ListboxDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultListboxDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isListboxDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): ListboxDocSectionId | null {
    ...
    if (section === undefined || !isListboxDocSectionId(section)) {
      return null;
    }
