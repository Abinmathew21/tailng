...
function isTreeDocSectionId(value: string): value is TreeDocSectionId {
  return treeDocSectionIds.includes(value as TreeDocSectionId);
}
...
export class TreePageComponent {
  ...
  public readonly activeSection = computed<TreeDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultTreeDocSection;
  });
  ...
  private resolveSectionFromUrl(rawUrl: string): TreeDocSectionId | null {
    ...
    if (section === undefined || !isTreeDocSectionId(section)) {
      return null;
    }
