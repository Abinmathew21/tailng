...
function isAvatarDocSectionId(value: string): value is AvatarDocSectionId {
  return avatarDocSectionIds.includes(value as AvatarDocSectionId);
}
...
export class AvatarPageComponent {
  ...
  public readonly activeSection = computed<AvatarDocSectionId>(() => {
    const section = this.resolveSectionFromUrl(this.currentUrl());
    return section ?? defaultAvatarDocSection;
  });
  ...
  public onSectionChange(value: string | number | null): void {
    if (typeof value !== 'string' || !isAvatarDocSectionId(value)) {
      return;
    }
  ...
  private resolveSectionFromUrl(rawUrl: string): AvatarDocSectionId | null {
    ...
    if (section === undefined || !isAvatarDocSectionId(section)) {
      return null;
    }
