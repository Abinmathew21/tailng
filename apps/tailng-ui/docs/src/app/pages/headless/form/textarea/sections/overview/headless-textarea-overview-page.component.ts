...
export class HeadlessTextareaOverviewPageComponent implements OnDestroy {
  ...
  private readonly tailwindExampleHtmlCode = [
    ...
    '  <div',
    '    tngInputGroup',
    '    class="min-h-[7.5rem] items-start gap-3 rounded-xl border border-[var(--tng-semantic-border-strong)] bg-[var(--tng-semantic-background-base)] px-3 py-3 shadow-none transition',
    ...
    '      rows="4"',
    '      aria-label="Release notes"',
    '      class="min-h-[6rem] min-w-0 flex-1 resize-y appearance-none border-0 bg-transparent p-0 text-[var(--tng-semantic-foreground-primary)] outline-none placeholder:text-[var(--tng-semantic-foreground-muted)] focus:outline-none focus:ring-0"',
    '      [value]="releaseNotes()"',
    '      (input)="onReleaseNotesInput($event)"',
    '    ></textarea>',
