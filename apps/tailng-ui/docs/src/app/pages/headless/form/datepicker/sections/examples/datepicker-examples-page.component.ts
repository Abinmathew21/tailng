...
function createScopedSharedCssCode(classes: ExampleClassNames): string {
  return [
    ...
    `.${classes.trigger} {`,
    '  flex: 0 0 auto;',
    '  min-width: calc(var(--tng-datepicker-nav-size) + 0.82rem);',
...
function createHeadlessTailwindHtmlCode(options: {
  ...
  return [
    ...
    '        >',
    '          <input',
    '            class="min-h-full min-w-0 flex-1 appearance-none border-0 bg-transparent px-[0.88rem] py-[0.72rem] text-[0.98rem] font-semibold tracking-[0.01em] text-slate-950 outline-none ring-0 focus:outline-none focus:ring-0 placeholder:font-medium placeholder:text-slate-400"',
    `            [tngDatepickerInput]="${bindings.controller}"`,
    '            type="text"',
    '            inputmode="numeric"',
    `            placeholder="${options.placeholder}"`,
    ...
    '            type="button"',
    '            tabindex="-1"',
    `            class="inline-flex min-w-[3.1rem] shrink-0 items-center justify-center gap-[0.28rem] border-l border-slate-300/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] pl-[0.54rem] pr-[0.9rem] text-slate-950 transition-colors duration-150 ${options.brandHover}"`,
    `            [tngDatepickerTrigger]="${bindings.controller}"`,
    `            aria-label="${options.triggerLabel}"`,
    '          >',
