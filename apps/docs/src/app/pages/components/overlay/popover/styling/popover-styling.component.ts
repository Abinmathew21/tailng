...
export class PopoverStylingComponent {
  readonly containerSlotHtml = computed(
    () => `<tng-popover [slot]="{ container: 'flex gap-2' }">...</tng-popover>`,
  );
  ...
  readonly themedHtml = computed(
    () => `
<tng-popover placement="bottom-start" [slot]="{ container: 'flex gap-2', trigger: 'rounded-md bg-primary', panel: 'p-3 rounded-lg border border-border bg-bg shadow-lg' }">
