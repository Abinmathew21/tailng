...
it('createActiveDescendantController: always includes host id attribute', () => {
  const controller = createActiveDescendantController({ hostId: 'listbox-1' });

  expect(controller.getHostAttributes().id).toBe('listbox-1');
});

it('createActiveDescendantController: tracks active descendant id', () => {
  const controller = createActiveDescendantController({ hostId: 'listbox-1' });
...
it('createActiveDescendantController: emits aria-activedescendant when active id exists', () => {
  const controller = createActiveDescendantController({ hostId: 'listbox-1' });
...
it('createActiveDescendantController: supports initial active id', () => {
...
it('createActiveDescendantController: clears active descendant and removes aria-activedescendant', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
    initialActiveId: 'option-1',
  });
...
it('createActiveDescendantController: supports explicit null updates', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
    initialActiveId: 'option-1',
  });
...
it('createActiveDescendantController: returns frozen host attributes objects', () => {
  const controller = createActiveDescendantController({ hostId: 'listbox-1' });
...
it('createActiveDescendantController: moves next and previous across configured item ids', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
...
it('createActiveDescendantController: loops by default', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
...
it('createActiveDescendantController: respects non-loop mode at boundaries', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
    itemIds: ['option-1', 'option-2'],
    loop: false,
...
it('createActiveDescendantController: skips disabled ids and rejects disabled setActiveId', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
    itemIds: ['option-1', 'option-2', 'option-3'],
...
it('createActiveDescendantController: clears active id when item list update removes it', () => {
  const controller = createActiveDescendantController({
    hostId: 'listbox-1',
    itemIds: ['option-1', 'option-2'],
