...
it('createFocusScope: defaults restoreFocus to true and trapFocus to false', () => {
...
it('createFocusScope: supports restoreFocus and trapFocus overrides', () => {
  const scope = createFocusScope({
    restoreFocus: false,
    trapFocus: true,
  });
...
it('createFocusScope: activates and deactivates with restore target handling', () => {
  const scope = createFocusScope();
...
it('createFocusScope: returns null when deactivating an inactive scope', () => {
  const scope = createFocusScope();
  expect(scope.deactivate()).toBeNull();
});

it('createFocusScope: ignores restore target when restoreFocus is disabled', () => {
...
it('createFocusScope: records focus with and without member restrictions', () => {
...
it('createFocusScope: registers and unregisters members with normalized ids', () => {
  const scope = createFocusScope();
