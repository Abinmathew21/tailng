...
it('createTypeaheadController: matches item by typed prefix', () => {
...
it('createTypeaheadController: matches case-insensitively', () => {
  const controller = createTypeaheadController({
    items: [
      { id: 'alpha', text: 'Alpha' },
      { id: 'beta', text: 'Beta' },
...
it('createTypeaheadController: buffers keys within timeout window', () => {
  const controller = createTypeaheadController({
    bufferResetMs: 50,
    items: [
      { id: 'alpha', text: 'Alpha' },
...
it('createTypeaheadController: resets buffer after timeout', () => {
  const controller = createTypeaheadController({
    bufferResetMs: 10,
    items: [
      { id: 'alpha', text: 'Alpha' },
...
it('createTypeaheadController: does not reset buffer at exact timeout boundary', () => {
  const controller = createTypeaheadController({
    bufferResetMs: 10,
    items: [
      { id: 'ab', text: 'Ab' },
...
it('createTypeaheadController: skips disabled matches and picks the next enabled item', () => {
  const controller = createTypeaheadController({
    items: [
...
it('createTypeaheadController: keeps current active id when there is no prefix match', () => {
  const controller = createTypeaheadController({
...
it('createTypeaheadController: reset clears the typed buffer', () => {
  const controller = createTypeaheadController({
