...
function resolveDateFormatter(date: Date, format: TngDateFormatToken | string, locale?: string): string {
  if (format === 'input') {
...
export const defaultDatepickerDateAdapter: TngDateAdapter<Date> = Object.freeze({
  addDays: (date, amount) => {
  ...
  addMonths: (date, amount) => {
    ...
    return createLocalDate(result.getFullYear(), result.getMonth(), Math.min(date.getDate(), lastDay));
  },
