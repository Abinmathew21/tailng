...
export function d(value: string): Date {
  const parsed = defaultDateRangePickerDateAdapter.parse(value, 'en-US');
  if (parsed === null) {
...
export function createController(
  config: Partial<TngDateRangePickerConfig<Date>> = {},
): TngDateRangePickerController<Date> {
  return createDateRangePickerController<Date>({
    closeOnSelect: false,
    locale: 'en-US',
...
): readonly TngDateRangePickerEvent<Date>[] {
  const events: TngDateRangePickerEvent<Date>[] = [];
  ...
  return Object.freeze({
    ...
    format: (date, format, locale) => {
      if (format === 'year-label') {
        return String(defaultDateRangePickerDateAdapter.getYear(date) + 543);
      }
